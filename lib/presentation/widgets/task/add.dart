...
class _TaskAddWidgetState extends ConsumerState<TaskAddWidget> {
  ...
  Widget _buildInputForm(BuildContext context, AddBloc.TaskAddBloc bloc) {
    return Padding(
          ...
          ElevatedButton(
            onPressed: () async {
              if (!(_nameKey.currentState?.validate() ?? false) ||
                  !(_descriptionKey.currentState?.validate() ?? false)) {
                return;
              }
  ...
  Widget _buildErrorState(
      BuildContext context, AddBloc.TaskAddBloc bloc, AddState.Error state) {
    return Padding(
      padding: const EdgeInsets.all(18.0),
      child: Column(
        children: [
